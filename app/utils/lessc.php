...
class lessc {
	...
	function parseChunk() {
		...
		if ($this->literal('}')) {
			...
			$hidden = true;
			if (!isset($block->args) && !empty($block->tags)) foreach ($block->tags as $tag) {
			...
			if(!empty($block->tags)) foreach ($block->tags as $tag) {
	...
	function mixImports($block) {
		$props = array();
		if(!empty($block->props)) foreach ($block->props as $prop) {
			if ($prop[0] == 'import') {
				list(, $path) = $prop;
